...
- 为啥bt是第6次tick就完成，而fsm是第7次


这个问题问得非常精准！这正是行为树（BT）的并行架构与状态机（FSM）的跳转逻辑在执行效率上的本质区别。

虽然两者的业务逻辑都是“连续看到 5 次人”，但由于它们**“做决定的时机”**不同，导致在时间轴上产生了一个 Tick 的偏差。


**在行为树（BT）中：**
BT 的并行节点在同一个 Tick 内会依次执行所有子节点。

第 2 次 Tick：

- 传感器节点先运行：写黑板 person_visible = True。

- 触发器节点接着运行：读黑板发现为 True，立刻把计数器加到 1。

结论：在 BT 里，“感知”和“计数”是同步的。所以到第 6 次 Tick 时，计数器已经完成了 2, 3, 4, 5, 6 这 5 次累加，刚好满 5 次。

**在有限状态机（FSM）中：**

FSM 通常遵循“感知 -> 逻辑判断 -> 状态切换”的模式，状态的切换往往发生在 Tick 的末尾，或者是为了保证稳定，在本 Tick 识别条件，下个 Tick 才进入新状态。
...
- 逻辑决定：下一次 Tick 切换到 DETECT_1。

结论：FSM 的状态跳转往往滞后于感知。它在第 2 次 Tick 看到人，但在第 3 次 Tick 才真正进入“计数为 1”的状态。这就导致它比 BT 晚了一个节拍。


Tick 序	传感器状态	行为树 (BT) 内部动作	FSM 内部动作
Tick 1	无人	计数=0，状态 Running	处于 IDLE，状态不变
Tick 2	有人	计数=1，状态 Running	识别到人，预备跳转
Tick 3	有人	计数=2，状态 Running	进入 DETECT_1 (计数=1)
Tick 4	有人	计数=3，状态 Running	进入 DETECT_2 (计数=2)
Tick 5	有人	计数=4，状态 Running	进入 DETECT_3 (计数=3)
Tick 6	有人	计数=5 -> 报 SUCCESS	进入 DETECT_4 (计数=4)
Tick 7	有人	(已结束)	进入 SUCCESS (计数=5)


作为老师，你可以这样给学生解释：

- BT 是“扁平化管理”：主管（Parallel）在同一个 tick 内把所有信息收集齐（传感器写、计数器读），当场就拍板。信息流是纵向实时的。

- FSM 是“层级审批”：你这一秒提交了申请（发现人），我需要处理一下，下一秒才给你发新的工牌（切换状态）。信息流是横向滞后的。

这就产生了一个有趣的架构结论： 行为树在处理这种“条件触发任务”时，响应速度通常比基于简单跳转的状态机快 1 个周期。在高速运行的机器人（如无人机避障）中，这一个 Tick（可能仅 10ms）往往就是避开障碍物与撞上去的区别。
